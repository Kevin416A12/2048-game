...
(define (mover-fila-derecha fila)
  (invertir
   (mover-fila-izquierda
    (invertir fila))))
; toma el primer elemento de cada fila, o sea la columna.
...
; ===============================
; aplica una función a cada fila de la matriz          ESTO ES MUY PRO
...
; Lo mismo pero para el puntaje

(define (combinar-fila-con-puntaje fila)
  (cond
    [(null? fila)
     (cons '() 0)]
    ...
    [(= (car fila) (cadr fila))
     (cons
      (cons (+ (car fila) (cadr fila))
            (car (combinar-fila-con-puntaje (cddr fila))))
      (+ (+ (car fila) (cadr fila))
         (cdr (combinar-fila-con-puntaje (cddr fila)))))]

    [else
     (cons
      (cons (car fila)
            (car (combinar-fila-con-puntaje (cdr fila))))
      (cdr (combinar-fila-con-puntaje (cdr fila))))]))

; Devuelve:
; (cons fila-final puntaje)
(define (mover-fila-izquierda-con-puntaje fila)
  (list
   (rellenar-con-ceros
    (car (combinar-fila-con-puntaje
          (mover-fila-izquierda-simple fila)))
    (contar-elementos fila))

   (cdr (combinar-fila-con-puntaje
         (mover-fila-izquierda-simple fila)))))

; Devuelve:
; (cons tablero puntaje)
(define (mover-izquierda-con-puntaje matriz)
  (cond
    [(null? matriz)
     (list '() 0)]
    ...
    [else
     (list
      (cons
       (car
        (mover-fila-izquierda-con-puntaje
         (car matriz)))
...
(define (mover-derecha-con-puntaje matriz)
  (list
   (aplicar-a-matriz
    (car
     (mover-izquierda-con-puntaje
      (aplicar-a-matriz matriz invertir)))
    invertir)
...
(define (mover-arriba-con-puntaje matriz)
  (list
   (transponer
    (car
     (mover-izquierda-con-puntaje
...
(define (mover-abajo-con-puntaje matriz)
  (list
   (transponer
    (car
